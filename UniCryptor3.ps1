...
# Encrypts a chunk using AES-GCM via the UCGcm helper
...
# Decrypts a chunk using AES-GCM via the UCGcm helper
...
# Creates a new SevenZipCompressor instance with configured options
...
# Registers an object event for 7z progress reporting
...
# Container format constants and utilities
class UCFormat {
    ...
    # Builds the container header from flags, nonce, and ciphertext length
    ...
    # Parses and validates the container header
    ...
    # Builds the AAD for each chunk: header bytes + chunk index
    ...
    # Builds the per-chunk nonce: base nonce + big-endian chunk counter
...
# Random number and password generation utilities
class UCRandom {
    static [int[]] $PasswordCharset = 49..57 + 65..72 + 74..78 + 80..82 + 84..90 + 97..104 + 106 + 107 + 109 + 110 + 112..122 + 33 + 35 + 36 + 38 + 43 + 45

    # Generates a random password from a printable ASCII character set
...
# Metadata serialization for original file attributes
class UCMetadata {
    # Builds a TLV metadata block: original length, creation time, last write time
...
# Envelope handling for key transport (RSA and password-based)
class UCEnvelope {
    # Derives a 16-byte key ID from a certificate thumbprint
    ...
    # Wraps the AES key with RSA-OAEP-SHA256 for each certificate
    ...
    # Parses the recipient envelope into individual entries
    ...
    # Attempts to unwrap the AES key using provided certificates
    ...
    # Builds a password envelope: salt + iteration count
...
# Container footer construction
class UCContainer {
    # Builds the footer with envelope, ciphertext length, and CRC8 checksum
...
class UniCryptor3 {
    ...
    # Adds encryption certificates from a list of thumbprints or X509Certificate2 objects
    ...
    # Writes an encrypted container to the output stream
    ...
    # Reads and validates container information from the stream
    ...
    # Decrypts a single chunk with authentication
    ...
    # Decrypts a container entirely into memory
    ...
    # Decrypts a container directly to a file path
    ...
    # Resolves the encryption key from the container (password or certificate-based)
    ...
    # Encrypts a string using configured certificates
    ...
    # Internal method for string encryption
    ...
    # Decrypts a string (certificate or password-based)
    ...
    # Internal method for file encryption
    ...
    # Encrypts a file using certificates
    ...
    # Internal method for file decryption
    ...
    # Decrypts a file (certificate-based)
    ...
    # Internal method for folder encryption
    ...
    # Encrypts a folder using certificates
    ...
    # Decrypts a folder using a password (wrapper)
    ...
    # Gets container information without requiring decryption
    ...
    # Waits for a 7z async operation to complete with progress reporting
    ...
    # Creates an encrypted 7z archive with certificate-based key protection
    ...
    # Internal method to retrieve the archive password from the container
    ...
    # Retrieves the password used to encrypt a 7z archive
    [string] GetArchivePassword([string]$ArchivePath) { return $this.GetArchivePasswordInternal($ArchivePath) }

    # Lists the contents of an encrypted 7z archive
...
# Encrypts a string (certificate or password-based)
...
# Encrypts a file or folder (certificate or password-based)
...
# Decrypts a file or folder (certificate or password-based)
...
# Gets container information for a file
...
# Retrieves the password from an encrypted 7z archive
